...
export const commands: Record<string, CommandDef<any>> = {
  ...
  'set-my-language': def(z.object({ language: zLang }).strict(), async ctx => { const staff = requireStaff(ctx.principal); await org.changeMyLanguage(staffEnv(staff), ctx.body); return { ok: true }; }),
